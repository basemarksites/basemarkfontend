...
import { Container, CardColumns, Card, CardImg, CardBody, CardTitle, Button, Col } from 'reactstrap'
...
export default class NewItems extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:3001/newItems', this.state.config)
            .then((response) => {
        ...
        if (localStorage.getItem('token')) {
            return (
                <div style={{ marginTop: "30px " }}>
                    ...
                    <Container>
                        <CardColumns>
                            {this.state.newItems.map((allProducts =>
                                <Card>
